...
type Portal = { id: number; key: string; name: string };
type Magazine = { id_magazine: string; name: string };
type PublicationRow = {
...
function publicationEditionLabel(p: PublicationRow): string {
...
function isMagazineAdvertService(serviceName: string): boolean {
...
const ServiceSelectModal: FC<ServiceSelectModalProps> = ({ open, onClose, onConfirm }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setPortals((portalList as Portal[]).filter((p) => p && typeof p.id === "number"));
      setMagazines((magazineList as Magazine[]).filter((m) => m && typeof m.id_magazine === "string"));
    });
  }, [open]);
  ...
  useEffect(() => {
    ...
    PublicationService.listPublicationsForMagazine(selectedMagazineId)
      .then((list: any[]) => {
        ...
        const normalized: PublicationRow[] = rows
                      ...
                      <div className="px-4 pt-4 pb-4 space-y-4 border-t border-gray-200/80">
                        ...
                        {isMagazineAdvert && selectedMagazinePublicationId && (
                          <div className="pt-3">
                            ...
                            <div className="space-y-2">
                              {PAGE_TYPES.map(({ pageType, slotKey }) => (
                                <button
                                  key={slotKey}
                                  type="button"
                                  onClick={() => handleSelectPage(pageType, slotKey, true)}
