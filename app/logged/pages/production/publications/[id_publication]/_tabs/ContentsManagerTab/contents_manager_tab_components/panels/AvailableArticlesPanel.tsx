...
function publicationIssueHref(publicationId: string): string {
  return `${BASE}/issues/${encodeURIComponent(publicationId)}`;
}

function PublicationLinksCell({ links }: { links: LinkedPublicationRow[] }) {
  if (!links.length) {
    return <span className="text-gray-500">Not in any publication</span>;
  }
        ...
        return (
          <div key={`${link.publication_id}-${label}`}>
            ...
            {link.is_current_publication ? (
              <span className="ml-1 rounded-full border border-blue-200 bg-blue-50 px-1.5 py-0.5 text-[10px] font-medium text-blue-700">
                This publication
              </span>
            ) : null}
    ...
    <div className="space-y-4">
      ...
      <section className="space-y-3">
        <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
          <div>
            <h3 className="text-sm font-semibold text-gray-800">Available portal articles</h3>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left font-medium">Magazine publication</th>
              ...
              <tbody>
                {available.items.map((row) => {
                  ...
                  return (
                    <tr key={rowKey} className="border-t border-gray-200 hover:bg-gray-50/60">
