...
/** Formats date as "dd mm yyyy" (day, month, year) for unambiguous display */
...
const PmDashboardPage: FC = () => {
  ...
  const eventsFromToday = allEvents.filter((e) => e.date >= todayDate);
  const eventsForTable = eventsFromToday.length > 0 ? eventsFromToday : allEvents;
  ...
  const getProjectTitle = (id: string) => projects.find((p) => p.id_project === id)?.title ?? id;

  const eventsByTab = useMemo(() => {
    return eventsForTable.filter((e) => e.event_state === eventStateTab);
  }, [eventsForTable, eventStateTab]);
  ...
  const filteredEventsByTab = useMemo(() => {
    ...
    if (filterMonth || filterYear.trim()) {
      ...
      if ((monthNum === null || (monthNum >= 1 && monthNum <= 12)) && (yearNum === null || (yearNum >= 1000 && yearNum <= 9999))) {
        result = result.filter((e) => {
          ...
          const matchMonth = !monthNum || eventMonth === monthNum;
          const matchYear = !yearNum || eventYear === yearNum;
    ...
    if (projQ) {
      result = result.filter((e) =>
        (projects.find((p) => p.id_project === e.id_project)?.title ?? e.id_project).toLowerCase().includes(projQ)
      );
    }
    const custQ = filterCustomer.trim().toLowerCase();
    if (custQ) {
      result = result.filter((e) =>
        (customers.find((c) => c.id_customer === e.id_customer)?.name ?? e.id_customer).toLowerCase().includes(custQ)
      );
    }
    return result;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filterType, filterMonth, filterYear, filterProject, filterCustomer, eventStateTab]);
  const getCustomerName = (id: string) => customers.find((c) => c.id_customer === id)?.name ?? id;
  ...
  const formatDateKeyFromDate = (d: Date) => {
  ...
  const eventTypeCardColor: Record<string, string> = {
    ask_materials: "bg-blue-200 text-blue-900 border-blue-300",
    send_preview: "bg-amber-200 text-amber-900 border-amber-300",
    publication_date: "bg-emerald-200 text-emerald-900 border-emerald-300",
  };
                ...
                return (
                  <div
                    ...
                    className={`aspect-square border rounded p-1 text-xs flex flex-col relative cursor-pointer transition-all hover:shadow-md ${
                      isToday ? "ring-1 ring-blue-600" : "border-gray-200"
                    }`}
                  >
                    ...
                    {hasEvents && (
                      <div className="flex-1 flex flex-col gap-0.5 mt-1 overflow-hidden overflow-y-auto">
                        {dayEvents.map((ev) => (
                          <div
                            key={ev.id_event}
                            className={`px-1 py-0.5 rounded text-[9px] font-medium border truncate ${eventTypeCardColor[ev.event_type] ?? "bg-gray-200 text-gray-800 border-gray-300"}`}
                            title={`${eventTypeLabel[ev.event_type] ?? ev.event_type} - ${getCustomerName(ev.id_customer)}`}
                          >
  ...
  return (
    <div className="flex flex-col w-full bg-white min-h-screen">
      <div className="text-center bg-blue-950/70 p-5 text-white">
        <p className="text-2xl">Project Management Dashboard</p>
      </div>
      ...
      <div className="flex flex-col gap-8 p-12">
                ...
                <div className="border border-gray-200 rounded-lg p-8">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xl text-gray-600 capitalize mt-1">
                  ...
                  <div className="mt-8">
                    ...
                    {getPmEventsForDate(selectedDay).length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {getPmEventsForDate(selectedDay).map((ev) => (
                          <div
                            key={ev.id_event}
                            onClick={() => router.push(`/logged/pages/pm/projects/${ev.id_project}`)}
                            className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer hover:bg-blue-50/50"
                          >
                            <h4 className="font-semibold text-lg text-gray-900 mb-2 hover:text-blue-600">
                              {getProjectTitle(ev.id_project)}
                            </h4>
                            <div className="flex flex-wrap gap-2 mb-2">
                              <span className={`px-2 py-0.5 rounded text-xs font-medium ${eventTypeCardColor[ev.event_type] ?? "bg-gray-200 text-gray-800"}`}>
        ...
        <div>
          ...
          <div className="mb-4 flex flex-wrap gap-6 items-end">
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-[140px]"
              >
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-[120px]"
              >
                <option value="">All</option>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-24"
              />
            </div>
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-[180px]"
              />
            </div>
            <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-row border-b border-gray-200 gap-1 mb-4">
            {(["pending", "done"] as EventStateTab[]).map((tab) => (
              <button
                key={tab}
                type="button"
                onClick={() => setEventStateTab(tab)}
                className={`px-6 py-3 font-medium rounded-t-lg transition-colors capitalize ${
                  eventStateTab === tab
          ...
          {filteredEventsByTab.length === 0 ? (
            <p className="py-6 text-center text-gray-500 text-sm border border-gray-200 rounded-lg bg-gray-50">
            ...
            <>
              ...
              <div className="flex items-center justify-between mt-4 px-2">
                <p className="text-sm text-gray-600">
                  Viewing {paginatedEvents.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, filteredEventsByTab.length)} of {filteredEventsByTab.length}
                </p>
