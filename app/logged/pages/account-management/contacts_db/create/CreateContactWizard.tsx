...
import { getProposalWizardHref } from "@/lib/account-management/proposalRoutes";
...
type CreateContactPageContentProps = {
  fromProposalId?: string;
};

const CreateContactPageContent: FC<CreateContactPageContentProps> = ({ fromProposalId }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const customerFromUrl = searchParams.get("customer")?.trim() ?? "";
  const proposalId = fromProposalId?.trim() ?? "";
  ...
  const [step, setStep] = useState<Step>(proposalId ? 2 : 1);
  ...
  useEffect(() => {
    if (!customerFromUrl || proposalId) return;
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const idCustomer = String(p?.id_customer ?? "").trim();
        if (!idCustomer) {
          setProposalLoadError("This proposal has no linked account.");
          return;
        }
        const cust = await CustomerService.getCustomerById(idCustomer);
        if (cancelled) return;
        const c = cust as {
          id_customer?: string;
          name?: string;
        ...
        const linked: CustomerRow = {
          id_customer: String(c.id_customer ?? idCustomer),
          name: String(c.name ?? ""),
          cif: String(c.cif ?? ""),
          country: String(c.country ?? ""),
        };
  ...
  const minStep: Step = proposalId ? 2 : 1;

  const goBack = () => {
    if (step > minStep) setStep((s) => (s - 1) as Step);
  };

  const resolveLinkedCustomerFields = () => {
    const id_customer = String(form.linkedCustomer?.id_customer ?? form.id_customer ?? "").trim();
    const company_name = String(
      form.linkedCustomer?.name ?? form.company_name ?? ""
    ).trim();
    return { id_customer, company_name };
  };

  const handleFinish = async () => {
    setSubmitError(null);
    const { id_customer, company_name } = resolveLinkedCustomerFields();

    if (!form.name.trim() || !form.email.trim()) {
      setSubmitError("Name and email are required.");
      return;
    }
    if (proposalId && !id_customer) {
      setSubmitError("Could not resolve the account from this proposal. Go back and try again.");
      return;
    }

    setSubmitting(true);
    try {
      const freshList = await ContactService.getAllContacts();
      const freshContacts = Array.isArray(freshList) ? freshList : [];
      let id_contact = String(form.id_contact ?? "").trim();
      if (!id_contact || freshContacts.some((c) => c.id_contact === id_contact)) {
        id_contact = generateNextContactId(freshContacts);
      }
      ...
      if (proposalId) {
        router.push(
          `${getProposalWizardHref(proposalId)}?new_contact=${encodeURIComponent(id_contact)}`
        );
      } else {
      ...
      const ax = e as { message?: string; data?: { message?: string } };
      const msg =
        ax?.data?.message ||
        ax?.message ||
        "Failed to create contact. Try again.";
  ...
  const pageHeading = proposalId ? "New contact for proposal" : "New contact";
  const backHref = proposalId
    ? getProposalWizardHref(proposalId)
    : "/logged/pages/account-management/contacts_db";
  ...
  const breadcrumbs = [
    ...
    ...(proposalId
      ? [
          ...
          { label: "Proposal", href: getProposalWizardHref(proposalId) },
        ]
      : []),
    { label: pageHeading },
  ];
  ...
  useEffect(() => {
    setPageMeta({
      ...
      buttons: [
        {
          label: proposalId ? "Back to proposal" : "Back",
          href: backHref,
        },
      ],
    });
  }, [setPageMeta, breadcrumbs, pageHeading, backHref, proposalId]);
  ...
  if (proposalLoading) {
    return (
      <PageContentSection>
        <p className="p-6 text-gray-600">Loading proposal account…</p>
      </PageContentSection>
    );
  }

  if (proposalLoadError) {
    return (
      <PageContentSection>
        <div className="p-6">
          ...
          <button
            type="button"
            onClick={() => router.push("/logged/pages/account-management/proposals")}
      ...
      <PageContentSection className="p-0">
        <div className="flex flex-col w-full mt-6 md:mt-8">
            ...
            <div className="flex items-center gap-4">
              {([1, 2, 3, 4] as Step[]).map((s) => (
                <React.Fragment key={s}>
                  <button
                    type="button"
                    onClick={() => s < step && s >= minStep && setStep(s)}
                    disabled={proposalId ? s < minStep : false}
                    className={`flex items-center justify-center w-10 h-10 rounded-full text-sm font-medium ${
                      proposalId && s < minStep ? "opacity-40 cursor-not-allowed" : ""
                    } ${
                      step === s
          ...
          <div className="bg-white rounded-b-lg overflow-hidden p-5 md:p-8 w-full max-w-full">
            {/* Step 1: Link to customer (optional; skipped when coming from a proposal) */}
            {step === 1 && !proposalId && (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {submitting
                      ? "Creating…"
                      : proposalId
                        ? "Create and go back to proposal"
                        : "Create contact"}
                  </button>
                </div>
              </form>
