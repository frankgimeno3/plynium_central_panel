...
const IdProduct: FC<IdProductProps> = ({ }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (companyId) {
        router.push(`/logged/pages/account-management/directory/companies/${encodeURIComponent(companyId)}`);
      } else {
        router.push('/logged/pages/account-management/directory/products');
      }
    } catch (error: unknown) {
      const msg =
        typeof error === 'string'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {formData.company?.trim() ? (
                <Link
                  href={`/logged/pages/account-management/directory/companies/${encodeURIComponent(formData.company)}`}
