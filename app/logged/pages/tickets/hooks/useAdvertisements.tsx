...
export type AdvertisementState = "pending" | "in process" | "accepted" | "rejected" | "expired";
...
export function AdvertisementsProvider({ children }: { children: ReactNode }) {
  ...
  const filteredAdvertisements = useMemo(() => {
    ...
    switch (currentTab) {
      ...
      case "in process":
        ...
        break;
      case "other":
        filtered = advertisements.filter((a) => !["pending", "in process"].includes(a.advReqState));
        break;
    }
    return filtered.sort((a, b) => new Date(b.senderDate).getTime() - new Date(a.senderDate).getTime());
  ...
  const updateAdvertisementState = useCallback((idAdvReq: string, newState: AdvertisementState) => {
    const stateMap: Record<AdvertisementState, NotificationState> = {
      pending: "pending",
    ...
    updateNotificationApi(idAdvReq, { state: stateMap[newState] })
      .then(() => {
        setAdvertisements((prev) => prev.map((a) => (idMatches(a.idAdvReq, idAdvReq) ? { ...a, advReqState: newState } : a)));
      })
      .catch(console.error);
  }, []);
  ...
  const addComment = useCallback(async (idAdvReq: string, content: string) => {
    ...
    setAdvertisements((prev) =>
      prev.map((a) => (idMatches(a.idAdvReq, idAdvReq) ? { ...a, commentsArray: [...a.commentsArray, newComment] } : a))
    );
  }, []);

  const getAdvertisementById = useCallback((idAdvReq: string) => advertisements.find((a) => a.idAdvReq === idAdvReq), [advertisements]);
  ...
  return <AdvertisementsContext.Provider value={value}>{children}</AdvertisementsContext.Provider>;
}
