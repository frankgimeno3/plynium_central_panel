...
export function OtherRequestsProvider({ children }: { children: ReactNode }) {
  ...
  const updateState = useCallback((id: string, newState: RequestState) => {
    ...
    updateNotificationApi(id, { state: stateMap[newState] })
      .then(() => {
        setRequests((prev) => prev.map((r) => (r.id === id ? { ...r, request_state: newState } : r)));
      })
      .catch(console.error);
  }, []);
  ...
  const addComment = useCallback(async (id: string, content: string) => {
    ...
    setRequests((prev) =>
      prev.map((r) => (r.id === id ? { ...r, commentsArray: [...(r.commentsArray ?? []), newComment] } : r))
    );
  }, []);

  const value = useMemo(() => ({ requests, updateState, addComment, getById }), [requests, updateState, addComment, getById]);

  return <OtherRequestsContext.Provider value={value}>{children}</OtherRequestsContext.Provider>;
}
