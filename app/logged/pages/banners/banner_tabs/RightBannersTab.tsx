...
export interface RightBannersTabProps {
    ...
    onChangeAppearanceWeight: (bannerId: string, section: 'home' | string, weight: AppearanceWeight) => void;
    onDeleteRightBanner: (bannerId: string, section: 'home' | string) => void;
...
const RightBannersTab: FC<RightBannersTabProps> = ({
    ...
    const sortedHomeRight = useMemo(
        () => [...homePageRightBanners].sort((a, b) => a.position - b.position),
        [homePageRightBanners]
    );
                                ...
                                <div className="flex flex-row items-center justify-between gap-4 flex-wrap">
                                    ...
                                    <div className="flex flex-row items-center gap-2 flex-shrink-0">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => onChangeAppearanceWeight(banner.id, 'home', e.target.value as AppearanceWeight)}
                                        ...
                                        <span className="text-sm text-gray-500 min-w-[4rem]">
                                            {getProbabilityPercent(homePageRightBanners, banner.id)}%
                                        </span>
                                        <button
                                            type="button"
                                            onClick={() => onDeleteRightBanner(banner.id, 'home')}
                                        ...
                                        <div key={banner.id} className="flex flex-col gap-2 p-4 border border-gray-200 rounded-lg">
                                            <div className="flex flex-row items-center justify-between gap-4 flex-wrap">
                                                <BannerCard
                                                    ...
                                                    onChangeRedirection={() => onChangeRedirection(banner.id, section.id, 'right')}
                                                />
                                                <div className="flex flex-row items-center gap-2 flex-shrink-0">
                                                    <label className="text-sm text-gray-600">Probability weight:</label>
                                                    <select
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => onDeleteRightBanner(banner.id, section.id)}
                                                        className="px-2 py-1 rounded bg-red-100 text-red-700 hover:bg-red-200 text-sm"
                                                    >
                                                        Remove
                                                    </button>
