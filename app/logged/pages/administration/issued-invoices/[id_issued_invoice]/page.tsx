...
const IssuedInvoiceDetailPage: FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await BillingService.getIssuedInvoiceById(id);
      const inv = data?.invoice;
      const orders = Array.isArray(data?.orders) ? data.orders : [];
      setInvoice(inv ? { ...inv, orders } : null);
    } catch {
      setInvoice(null);
    } finally {
      setLoading(false);
          ...
          <div className="flex flex-col w-full">
            <div className="bg-white rounded-b-lg overflow-hidden p-6">
              <p className="text-gray-500">
                {loading ? "Loading issued invoice…" : `Issued invoice not found: ${id}`}
              </p>
            </div>
          </div>
        </PageContentSection>
          ...
          <ul className="list-disc list-inside space-y-1 text-sm text-gray-700">
            {invoice.orders.map((o) => (
              <li key={o.order_code}>
                ...
                {" — "}
                {formatAdminDate(o.collection_date)} ·{" "}
                {o.payment_status ?? "—"} · €{o.amount_eur.toLocaleString()}
              </li>
            ))}
          </ul>
          </div>
