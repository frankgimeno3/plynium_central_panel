...
const ArticleFilter: FC<ArticleFilterProps> = ({ selectedPortalNames = [] }) => {
  ...
  const getFilterHref = () => {
    if (selectedFilter === "portal") {
      ...
      return `/logged/pages/contents/articles?portalNames=${encodeURIComponent(portalChecklist.join(","))}`;
    }
    if (!isFilterButtonEnabled) return "#";
    const searchParam = `${selectedFilter}__${filterValue.trim()}`;
    return `/logged/pages/contents/articles/search/${encodeURIComponent(searchParam)}`;
  };
