...
export default function AdvertisementDetailPage() {
  ...
  // Handle both string and array cases for Next.js params
  ...
  useEffect(() => {
    ...
    // Decode the advertisement ID in case it's URL encoded, and trim whitespace
    const decodedAdvReqId = decodeURIComponent(advReqId).trim();
    
    // Try to find the advertisement - use exact match first
    ...
    if (foundAdvertisement) {
      ...
      // Log for debugging
      console.error('Advertisement request not found. Looking for:', decodedAdvReqId);
      console.error('Available advertisement IDs:', advertisements.map(a => a.idAdvReq));
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      // The advertisement will be updated via the useEffect that watches advertisements
      setNewComment('');
    } catch (error) {
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
        ...
        <div className="mb-6">
          ...
          <button
            ...
            className={`
              mt-3 px-4 py-2 rounded-md text-white font-medium
