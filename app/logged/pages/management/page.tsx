...
const ManagementDashboardPage: FC = () => {
  ...
  const goToCurrentMonth = () => setCurrentMonth(new Date(new Date().getFullYear(), new Date().getMonth(), 1));
  ...
  const eventTypeLabel: Record<string, string> = { ask_materials: "Ask materials", send_preview: "Send preview", publication_date: "Publication date" };
  const eventTypeCardColor: Record<string, string> = { ask_materials: "bg-blue-200 text-blue-900 border-blue-300", send_preview: "bg-amber-200 text-amber-900 border-amber-300", publication_date: "bg-emerald-200 text-emerald-900 border-emerald-300" };
  ...
  const renderMonth = (month: Date) => {
    ...
    return (
      <div className="flex flex-col">
        <div className="text-center mb-4"><h3 className="text-lg font-semibold text-gray-900">{MONTH_NAMES[monthIndex]} {year}</h3></div>
        <div className="grid grid-cols-7 gap-1 mb-2">{WEEK_DAYS.map((day) => <div key={day} className="text-center text-xs font-medium text-gray-600 py-1">{day}</div>)}</div>
        {weeks.map((week, weekIndex) => (
          <div key={weekIndex} className="grid grid-cols-7 gap-1">
            {week.map((day, dayIndex) => {
              ...
              return (
                <div key={dayIndex} onClick={() => handleDayClick(date)} className={`aspect-square border rounded p-1 text-xs flex flex-col relative cursor-pointer transition-all hover:shadow-md ${isToday ? "ring-1 ring-blue-600" : "border-gray-200"}`}>
                  ...
                  {hasEvents && (
                    <div className="flex-1 flex flex-col gap-0.5 mt-1 overflow-hidden overflow-y-auto">
                      {dayEvents.map((ev) => (
                        <div key={ev.id_event} className={`px-1 py-0.5 rounded text-[9px] font-medium border truncate ${eventTypeCardColor[ev.event_type] ?? "bg-gray-200 text-gray-800 border-gray-300"}`} title={`${eventTypeLabel[ev.event_type] ?? ev.event_type} - ${getCustomerName(ev.id_customer)}`}>
  ...
  return (
    <div className="flex flex-col w-full bg-white min-h-screen">
      <div className="text-center bg-blue-950/70 p-5 text-white"><p className="text-2xl">Management Dashboard</p></div>
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  <button type="button" onClick={() => navigateMonth("prev")} className="px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded text-sm">←</button>
                  <button type="button" onClick={goToCurrentMonth} className="px-3 py-1 bg-blue-100 hover:bg-blue-200 text-blue-800 rounded text-sm">Current month</button>
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={() => navigateMonth("next")} className="px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded text-sm">→</button>
                </div>
                {renderMonth(month2)}
              </div>
            </div>
                    ...
                    {getPmEventsForDate(selectedDay).length === 0 ? <p className="text-gray-500">No events scheduled for this day</p> : (
                      <div className="space-y-4">
                        {getPmEventsForDate(selectedDay).map((ev) => (
                          <div key={ev.id_event} onClick={() => router.push(`/logged/pages/management/sm/projects/${ev.id_project}`)} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer hover:bg-blue-50/50">
                ...
                <table className="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {paginatedEvents.map((ev) => (
                      <tr key={ev.id_event} onClick={() => router.push(`/logged/pages/management/sm/projects/${ev.id_project}`)} className="hover:bg-blue-50/80 cursor-pointer">
