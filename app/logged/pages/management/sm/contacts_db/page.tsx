  ...
  return (
    <div className="flex flex-col w-full min-w-0 bg-white">
      <div className="w-full flex items-center justify-center gap-3 flex-wrap bg-blue-950/70 p-5 text-white">
        ...
        <Link
          href="/logged/pages/management/sm/contacts_db/create"
          className="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-sm font-medium transition-colors"
        >
          Nuevo contacto
        </Link>
        <Link
          href="/logged/pages/management/sm/contacts_db/mass-ops/import"
          className="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-sm font-medium transition-colors"
        >
          Import
        </Link>
        <Link
          href="/logged/pages/management/sm/contacts_db/mass-ops/export"
          className="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-sm font-medium transition-colors"
        >
          Export
        </Link>
          ...
          <table className="w-full min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered.map((c) => (
                <tr
                  key={c.id_contact}
                  onClick={() => router.push(`/logged/pages/management/sm/contacts_db/${c.id_contact}`)}
                  className={rowClass}
                >
