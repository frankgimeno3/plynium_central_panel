...
export function buildPublicationSearchHeading(filters: Record<string, string>): string {
  const filterDescriptions: string[] = [];
  if (filters.revista) filterDescriptions.push(`Magazine: ${filters.revista}`);
  if (filters.portalNames) {
    filterDescriptions.push(`Portal(s): ${filters.portalNames.split(',').map((s) => s.trim()).filter(Boolean).join(', ')}`);
  }
  if (filters.numero) filterDescriptions.push(`Number: ${filters.numero}`);
  if (filters.dateFrom) {
    const [year, month] = filters.dateFrom.split('-');
    const monthName = month ? monthNames[month] || month : '';
    if (month && year) filterDescriptions.push(`From: ${monthName} ${year}`);
  }
  if (filters.dateTo) {
    const [year, month] = filters.dateTo.split('-');
    const monthName = month ? monthNames[month] || month : '';
    if (month && year) filterDescriptions.push(`To: ${monthName} ${year}`);
  }
  return filterDescriptions.length > 0 ? `Publications: ${filterDescriptions.join(', ')}` : 'Search results';
}

interface PublicationSearchClientProps {
  filteredPublications: any[];
  filters: Record<string, string>;
  /** When provided, the layout is rendered by the parent (e.g. PageContentLayout). Only the grid is rendered here. */
  renderLayout?: (heading: string, filterNode: ReactNode, contentNode: ReactNode) => ReactNode;
}

const PublicationSearchClient: FC<PublicationSearchClientProps> = ({ filteredPublications, filters, renderLayout }) => {
  const heading = buildPublicationSearchHeading(filters);
  ...
  if (renderLayout) {
    return <>{renderLayout(heading, filterNode, contentNode)}</>;
  }
