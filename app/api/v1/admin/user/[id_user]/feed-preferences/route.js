...
import {
  getUserByIdFromRds,
  getUserFeedPreferenceRowsFromRds,
...
function getIdUserFromRequest(request) {
  ...
  const match = url.pathname.match(/\/api\/v1\/admin\/user\/([^/]+)\/feed-preferences/);
...
export const GET = createEndpoint(async (request) => {
  ...
  const rows = await getUserFeedPreferenceRowsFromRds(user.id);
